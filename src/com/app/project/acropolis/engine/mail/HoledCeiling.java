		...
		try{
					...
					e.getMessage().getFrom().getAddr().equalsIgnoreCase(debug))
			{
				if(e.getMessage().getSubject().equals(forcedCollection))
				{
					...
					//ControlledAccessException on BlackBerryLocationProvider
//					if(!LocationCode.Check_NON_CAN_Operator())
//					{
//						new Thread(new LocalHandler(false)).start();
//						new Logger().LogMessage("local started");
//					}
				...
				if(e.getMessage().getSubject().equals(forcedUpdate))
				{
					...
					ApplicationManager.getApplicationManager().postGlobalEvent(UPDATE_GUID, 0, 0, 
